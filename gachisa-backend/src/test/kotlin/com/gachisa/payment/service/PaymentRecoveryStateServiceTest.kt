...
class PaymentRecoveryStateServiceTest {
    ...
    @Mock lateinit var refundCompletionService: RefundCompletionService
    @Mock lateinit var timeProvider: TimeProvider
    @Mock lateinit var orderService: OrderService
    private val retryPolicy: PgRetryPolicy = PgRetryPolicy()
    private lateinit var stateService: PaymentRecoveryStateService

    @BeforeEach
    fun setUp() {
        stateService = PaymentRecoveryStateService(
                paymentRepository, attemptRepository, participationService, refundCompletionService,
                timeProvider, retryPolicy, orderService)
    }

    @Test
    fun doneStatusRecoversPaymentAndParticipation() {
        val payment = payment()
        val attempt = processingAttempt()
        mockLocked(payment, attempt)
        given(timeProvider.now()).willReturn(NOW)
        given(participationService.getPaymentInfo(10L))
                .willReturn(ParticipationPaymentInfo(10L, 20L, 30L, 1, false))
        ...
        val response = stateService.apply(2L, result("DONE"))

        assertThat(response.orderId).isEqualTo(100L)
        assertThat(payment.status).isEqualTo(PaymentStatus.PAID)
        assertThat(attempt.status).isEqualTo(PaymentAttemptStatus.PAID)
    ...
    @Test
    fun abortedStatusFailsOnlyAttemptSoAnotherAttemptCanBeCreated() {
        val payment = payment()
        val attempt = processingAttempt()
        mockLocked(payment, attempt)
        given(timeProvider.now()).willReturn(NOW)
        ...
        assertThat(attempt.failureCode).isEqualTo("TOSS_PAYMENT_ABORTED")
    }

    @Test
    fun expiredStatusExpiresOnlyAttempt() {
        val payment = payment()
        val attempt = processingAttempt()
        mockLocked(payment, attempt)
        given(timeProvider.now()).willReturn(NOW)
        ...
        assertThat(payment.status).isEqualTo(PaymentStatus.READY)
        assertThat(attempt.status).isEqualTo(PaymentAttemptStatus.EXPIRED)
    }

    private fun mockLocked(payment: Payment, attempt: PaymentAttempt) {
    ...
    private fun processingAttempt(): PaymentAttempt {
    ...
    private fun result(status: String): PgPaymentQueryResult {
        return PgPaymentQueryResult("payment-key", "gachisa_order", 12_600, status,
