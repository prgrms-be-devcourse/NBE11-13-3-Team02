...
**브랜치 이름은 `feat/` 로 통일한다.** 지금 `feat/` 와 `feature/` 가 섞여 있다.
...
**모든 PR 의 base 는 `dev`.** `main` 은 아직 `first commit` 하나뿐이고 `dev` 가 39커밋
앞서 있다. 발표 전에 한 번 맞출지 팀에서 정해야 한다.
